...
interface RawPeer {
  ...
  remote_ip: string;
  is_outbound: boolean;
  node_info: { moniker: string; network: string; listen_addr: string; version: string; };
}
...
async function fetchPeers() {
  ...
  try {
    ...
    const rawPeers: RawPeer[] = (data?.result?.peers || []).filter((p: any) => p && p.node_id);
    ...
    // 2) Önce GeoIP olmadan göster
    peers.value = rawPeers.map(p => rawToPeer(p, null));

    // 3) GeoIP — lumera RPC nginx proxy üzerinden ip-api.com
    const ips = [...new Set(rawPeers.map(p => p.remote_ip).filter(ip => ip && !ip.includes(':')))];
    ...
    // 4) GeoIP ile güncelle
    peers.value = rawPeers.map(p => rawToPeer(p, geoMap[p.remote_ip] || null));
...
async function renderMap() {
  ...
  peers.value.forEach(peer => {
    ...
    const flag = peer.countryCode && peer.countryCode !== 'xx'
          ...
          <div v-for="peer in filtered" :key="peer.node_id"
            ...
            <div class="flex items-center gap-2 min-w-0">
              ...
              <span class="font-mono text-[8px] hidden group-hover:inline truncate" style="color:var(--text-muted);">
                {{ peer.node_id ? peer.node_id.slice(0, 12) : '' }}...
              </span>
            </div>
