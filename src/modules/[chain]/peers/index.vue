...
async function fetchPeers() {
  ...
  try {
    ...
    // 2) Önce peer listesini GeoIP olmadan göster (harita hemen açılsın)
    peers.value = rawPeers.map(p => {
      const addr = p.node_info?.listen_addr || '';
      const port = (addr.match(/:(\d+)$/) || [])[1] || '26656';
      return {
        node_id: p.node_id, moniker: p.node_info?.moniker || 'unknown',
        ip: p.remote_ip, port, country: '...', countryCode: 'xx',
        city: '', org: '...', lat: 0, lon: 0,
        isOutbound: p.is_outbound,
        version: p.node_info?.version || '', network: p.node_info?.network || '',
      } as PeerGeo;
    });

    // 3) GeoIP — freeipapi.com HTTPS ücretsiz
    const ips = [...new Set(rawPeers.map(p => p.remote_ip))];
    const geoMap: Record<string, any> = {};
    const batchSize = 5;
    for (let i = 0; i < ips.length; i += batchSize) {
      await Promise.allSettled(
        ips.slice(i, i + batchSize).map(async ip => {
          try {
            const r = await fetch(`https://freeipapi.com/api/json/${ip}`);
            if (r.ok) {
              const g = await r.json();
              if (g.ipAddress) geoMap[ip] = g;
            }
          } catch {}
        })
      );
    ...
    // 4) GeoIP verisini peer'lara uygula
    peers.value = peers.value.map(p => {
      const g = geoMap[p.ip];
      if (!g) return p;
      return {
        ...p,
        country:     g.countryName  || 'Unknown',
        countryCode: (g.countryCode || 'XX').toLowerCase(),
        city:        g.cityName     || '',
        org:         g.isp || g.asn || 'Unknown',
        lat:         g.latitude     || 0,
        lon:         g.longitude    || 0,
      };
    });
