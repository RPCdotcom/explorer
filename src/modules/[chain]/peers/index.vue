...
const rpcEndpoint = computed(() => {
  ...
  // Format 1: { rpc: ["https://..."] }  (rpcdot config format)
  if (Array.isArray(current.rpc) && current.rpc.length > 0) {
    return current.rpc[0];
  }
  // Format 2: { endpoints: { rpc: [{ address: "https://..." }] } }  (ping.pub format)
  const rpcList = current.endpoints?.rpc;
  if (rpcList && rpcList.length > 0) {
    return rpcList[0].address || rpcList[0];
  }
  return '';
});
