...
// RPC formatları farklı olabilir — her ikisini de destekliyoruz
// Lumera: { node_info: { id, listen_addr }, is_outbound }  — remote_ip YOK
// Republic: { node_id, remote_ip, node_info: { ... }, is_outbound }
interface RawPeer {
  node_id?: string;
  remote_ip?: string;
  is_outbound: boolean;
  node_info: { id?: string; moniker?: string; network?: string; listen_addr?: string; version?: string; };
}

interface PeerGeo {
  node_id: string; moniker: string; ip: string; port: string;
  country: string; countryCode: string; city: string; org: string;
  lat: number; lon: number; isOutbound: boolean; version: string; network: string;
}
...
const GEOIP_URL = 'https://lumera-testnet-rpc.rpcdot.com/geoip/batch?fields=status,country,countryCode,city,org,lat,lon,query';

const rpcEndpoint = computed(() => {
  const c = blockchain.current as any;
  if (!c) return '';
  if (Array.isArray(c.rpc) && c.rpc.length > 0) return c.rpc[0];
  const list = c.endpoints?.rpc;
...
const filtered = computed(() => {
  ...
  return peers.value.filter(p =>
    p.moniker.toLowerCase().includes(kw) || p.ip.includes(kw) ||
    p.country.toLowerCase().includes(kw) || p.org.toLowerCase().includes(kw) ||
...
function parseListenAddr(addr: string): { ip: string; port: string } {
  const clean = (addr || '').replace(/^tcp:\/\//, '');
  const last  = clean.lastIndexOf(':');
  if (last < 0) return { ip: clean, port: '26656' };
  return { ip: clean.slice(0, last), port: clean.slice(last + 1) };
}

function rawToPeer(p: RawPeer, geo: any): PeerGeo {
  const { ip: listenIp, port } = parseListenAddr(p.node_info?.listen_addr || '');
  const ip     = p.remote_ip || listenIp || '';
  const nodeId = p.node_id   || p.node_info?.id || '';
  return {
    node_id:     String(nodeId),
    moniker:     String(p.node_info?.moniker  || 'unknown'),
    ip:          String(ip),
    port:        String(port),
    country:     String(geo?.country     || 'Unknown'),
    countryCode: String(geo?.countryCode || 'XX').toLowerCase(),
    city:        String(geo?.city        || ''),
    org:         String(geo?.org         || 'Unknown'),
    lat:         Number(geo?.lat         || 0),
    lon:         Number(geo?.lon         || 0),
    isOutbound:  Boolean(p.is_outbound),
    version:     String(p.node_info?.version || ''),
    network:     String(p.node_info?.network || ''),
  };
}
...
async function fetchPeers() {
  ...
  try {
    ...
    const rawPeers: RawPeer[] = (data?.result?.peers || []).filter(
      (p: any) => p && (p.node_id || p.node_info?.id)
    );
    ...
    // Hemen göster (GeoIP olmadan)
    peers.value = rawPeers.map(p => rawToPeer(p, null));

    // GeoIP — nginx proxy üzerinden
    const allIps = rawPeers.map(p => {
      const ip = p.remote_ip || parseListenAddr(p.node_info?.listen_addr || '').ip;
      return ip;
    }).filter(ip => ip && !ip.includes(':') && ip !== '0.0.0.0');
    ...
    for (const chunk of chunks) {
      try {
        ...
        if (r.ok) {
          ...
          list.forEach(g => { if (g?.status === 'success') geoMap[g.query] = g; });
        }
      } catch {}
    }
    ...
    peers.value = rawPeers.map(p => {
      const ip  = p.remote_ip || parseListenAddr(p.node_info?.listen_addr || '').ip;
      return rawToPeer(p, geoMap[ip] || null);
    });
...
async function renderMap() {
  ...
  peers.value.forEach(peer => {
    ...
    const cc   = peer.countryCode && peer.countryCode !== 'xx' ? peer.countryCode : '';
    const flag = cc ? `<img src="https://flagcdn.com/16x12/${cc}.png" style="vertical-align:middle;margin-right:4px;"/>` : '';
    L.marker([peer.lat + jitter, peer.lon + jitter], { icon }).addTo(leafletMap).bindPopup(`
      <div style="font-family:'JetBrains Mono',monospace;font-size:11px;min-width:200px;">
        <div style="font-weight:700;margin-bottom:4px;color:#00e676;">${flag}${esc(peer.moniker)}</div>
        <div style="color:#aaa;margin-bottom:2px;"><span style="color:#555;">IP</span> ${esc(peer.ip)}:${peer.port}</div>
        <div style="color:#aaa;margin-bottom:2px;"><span style="color:#555;">NODE ID</span><br/><span style="font-size:9px;word-break:break-all;">${esc(peer.node_id)}</span></div>
        <div style="color:#aaa;margin-bottom:2px;"><span style="color:#555;">LOCATION</span> ${peer.city ? esc(peer.city)+', ' : ''}${esc(peer.country)}</div>
        <div style="color:#aaa;margin-bottom:2px;"><span style="color:#555;">PROVIDER</span> ${esc(peer.org)}</div>
        <div style="color:#aaa;"><span style="color:#555;">DIR</span> <span style="${peer.isOutbound ? 'color:#00e676' : 'color:#888'}">${peer.isOutbound ? '▲ OUT' : '▼ IN'}</span></div>
      </div>`, { maxWidth: 280 });
  });
}
...
function providerColor(org: string): string {
  ...
  if (o.includes('google'))                          return '#44aaff';
...
<template>
  <div>
    ...
    <div v-if="error" class="mb-4 px-4 py-2 font-mono text-xs flex gap-2"
      ...
      <span>[!]</span><span>{{ $t('peers.error') }}: {{ error }}</span>
    </div>
    ...
    <div v-if="peers.length">
      ...
      <div style="border:1px solid var(--border);overflow:hidden;">
        ...
        <div class="overflow-y-auto" style="max-height:480px;">
          <div v-for="peer in filtered" :key="peer.node_id || peer.ip"
            ...
            <div class="flex items-center gap-1.5 truncate">
              <img v-if="peer.countryCode && peer.countryCode !== 'xx'"
                :src="`https://flagcdn.com/16x12/${peer.countryCode}.png`"
                style="width:16px;height:12px;flex-shrink:0;" />
